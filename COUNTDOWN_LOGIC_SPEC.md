...
日次ダッシュボードは計算対象日を `reading_date` として返します。保存済みデータの `reading_date` が表示日と一致しない、または日付がない場合は表示日分を再取得します。安全策として、`impact_end_date` が表示日より前の項目はフロント側でも除外します。
...
短期・中長期のコメント判定では、表示対象になったアスペクトの `Pressure_Score` 絶対値をグループ別に合算します。

```text
短期:   0〜59 = low / 60〜159 = moderate / 160以上 = high
中長期: 0〜159 = low / 160〜399 = moderate / 400以上 = high
```

短期＋中長期の全体枠には合算スコアを持たせません。短期と中長期それぞれの `level` の組み合わせだけで全体メッセージを分岐します。
