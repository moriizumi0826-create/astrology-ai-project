...
## 7. Recommended_Action 生成ルール

`Recommended_Action` は、タイムラインや短時間の行動提案で使う日常的な推奨アクション文です。

既存の `Advised_Task` は短い命令形のタスク、`Text_Description` は占星術的な解釈文として扱い、`Recommended_Action` はその中間の粒度で「その時間帯に何をすると安定するか」を自然文で示します。

### 7-1. 基本方針
1. `T_Planet`, `N_Planet`, `Aspect_Angle`, `Category`, `Orb_Status`, `T_Retrograde_Flag` を反映すること。
2. 文体は現実的で、日常の行動に落とし込める内容にすること。
3. 詩的、ドラマティック、大げさな表現は禁止。
4. 文章量は 45〜75 文字程度を目安にすること。
5. 文末は原則として「〜に向く時間帯です。」「〜が安定します。」など、行動提案として読みやすい形にすること。

### 7-2. 文体例
- 判断速度と整合性が噛み合いやすく、設計や分析の骨子づくりに向く時間帯です。
- 集中の波がやや分散しやすいため、整理やレビューのような粒度の揃った作業が安定します。
- 感情と言語化が連動しやすく、面談準備や日報、関係調整のメモ作成に向いています。

### 7-3. 要素の反映
- `T_Planet = TRANSIT_SUN`: 意思決定、自己表現、主体性、目的意識を扱う。
- `T_Planet = TRANSIT_MOON`: 感情の反応、生活リズム、安心感、短時間の揺れを扱う。
- `Aspect_Angle = 0`: テーマが強まり、集中しやすい流れとして扱う。
- `Aspect_Angle = 60 / 120`: 連携、安定、自然な進行として扱う。
- `Aspect_Angle = 90 / 180`: ズレ、対比、確認、整理が必要な流れとして扱う。
- `Aspect_Angle = 150`: 異なるリズムの調整、手順の見直し、習慣化として扱う。
- `Orb_Status = Applying`: 先に準備を整える、これから強まる流れを見越すニュアンスを入れる。
- `Orb_Status = Separating`: 反応を振り返る、結果を整理する、定着させるニュアンスを入れる。
- `T_Retrograde_Flag = 1`: 過去の手順、再確認、見直しのニュアンスを入れる。

### 7-4. Category 別の行動粒度
- `General`: 今日の優先順位、メモ、最初の一手、日常の整え方。
- `Work`: 資料整理、判断材料の洗い出し、進行確認、設計や分析。
- `Love`: 返信内容の整理、温度差の確認、対話準備、関係調整。
- `Health`: 休憩計画、睡眠、食事、水分補給、生活リズムの調整。
- `Money`: 支出確認、予算メモ、買い物判断、契約や支払いの見直し。
