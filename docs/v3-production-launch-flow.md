...
2026-09-23に本番プロジェクトの「Database → Backups」を確認。現在のFreeプランには自動バックアップがなく、時点復元（PITR）も利用できない。Proへの変更は行っていない。公開前に、自動バックアップを利用するプランへ変更するか、運用者が安全に取得・保管・復元検証できる手動バックアップを構築するか選ぶ必要がある。バックアップ未確定のまま実額決済・一般公開へ進まない。
...
3. Supabaseのバックアップ方法（Proの自動バックアップか、保管・復元検証を含む手動方式）を決める。障害・負荷、通知・外部死活監視、復旧手順も実額決済より前に確認する。
