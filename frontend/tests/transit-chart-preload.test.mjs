...
test("paid Horoscope playback spans month/year/leap boundaries from the selected date", () => {
  assert.deepEqual(buildTransitPlaybackDates("2026-12-31", 3), ["2026-12-31", "2027-01-01", "2027-01-02"]);
  assert.deepEqual(buildTransitPlaybackDates("2028-02-28", 3), ["2028-02-28", "2028-02-29", "2028-03-01"]);
  const year = buildTransitPlaybackDates("2026-09-17", 365);
  assert.equal(year.length, 365);
  assert.equal(year[0], "2026-09-17");
  assert.equal(year.at(-1), "2027-09-16");
});
