...
from io_utils import ensure_output_dir, resolve_existing_path, read_csv_rows
...
def main() -> None:
    ...
    # Load Dynamic Inputs
    raw_angles = load_angles()
    has_birth_time = infer_has_birth_time(raw_angles)

    planets = load_planets(has_birth_time=has_birth_time)
    angles = raw_angles if has_birth_time else []
    aspects = load_aspects()
    nodes = load_nodes(has_birth_time=has_birth_time)
    has_north_node = any(
        (getattr(n, "name_en", "") == "North Node")
        or ("ヘッド" in str(getattr(n, "name_ja", "") or ""))
        or ("ノースノード" in str(getattr(n, "name_ja", "") or ""))
        for n in nodes
    )
    has_south_node = any(
        (getattr(n, "name_en", "") == "South Node")
        or ("テール" in str(getattr(n, "name_ja", "") or ""))
        or ("サウスノード" in str(getattr(n, "name_ja", "") or ""))
        for n in nodes
    )
    ...
    print(f"north node exists: {has_north_node}")
    print(f"south node exists: {has_south_node}")

    for i, n in enumerate(nodes, 1):
        print(
            f"node[{i}]: "
            f"name_en={getattr(n, 'name_en', '')}, "
            f"name_ja={getattr(n, 'name_ja', '')}, "
            f"sign_ja={getattr(n, 'sign_ja', '')}, "
            f"house={getattr(n, 'house', '')}"
        )

    if not has_north_node:
        print("WARNING: North Node (ドラゴンヘッド) is missing. Node sections may not render correctly.")
    ...
    has_north_node = any(n.name_en == "North Node" for n in nodes)
    has_south_node = any(n.name_en == "South Node" for n in nodes)
    ...
    narrative_section_plan = build_narrative_section_plan(
        bundle, narrative_bundle, narrative_plan, ctx
    )
    ...
    # Render Reading
    result_text = render_full_reading(
        ctx=ctx,
    ...
    print("出生時間ありモード:" if has_birth_time else "出生時間不明モード:")
    print("----------")
    print(result_text)
